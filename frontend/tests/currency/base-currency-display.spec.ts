...
test.describe('View All Values in Base Currency', () => {
    ...
    test('should not show manual currency selection in portfolio editor', async ({ page }) => {
        ...
        const currencyDropdown = page.locator('.modal-box select').filter({ hasText: /USD|EUR|Currency/i });
    ...
    test('should display portfolio totals in JPY', async ({ page }) => {
        ...
        // Total should be in ¥
        // Check the total on the main dashboard (HoldingsByAccount uses .bg-base-300 for total)
        await expect(page.locator('.bg-base-300').getByText(/¥1,000,000/i).last()).toBeVisible({ timeout: 5000 });
    });
    ...
    test('should show account limits in JPY', async ({ page }) => {
        ...
        // Account usage should show in ¥. Limit might vary (1.2M, 1.8M, 2.4M), so match pattern.
        await expect(page.getByText(/¥900,000.*¥[0-9,]+/).first()).toBeVisible({ timeout: 5000 });
    });
});
